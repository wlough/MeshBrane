import numpy as np
from convergence_tests.ply_utils import HalfEdgeMeshData
...
class Brane:
    ...
    def _get_faces(self):
        return np.array([self.get_face(f) for f in range(len(self.F_edge))], dtype=np.uint32)
    ...
    def get_cot_laplacian(self):
        """L_ij(y_j-y_i)"""
        Nvertices = len(self.V)
        # Initialize a list to store the non-zero elements and their indices
        data = []
        row_indices = []
        col_indices = []

        # Add elements to the matrix
        # For example, to add the element 1.0 at row 0, column 0:
        # data.append(1.0)
        # row_indices.append(0)
        # col_indices.append(0)
        for row in range(Nvertices):
            neighbors = self.one_ring_neighbors(row)
