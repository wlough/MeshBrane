...
from convergence_tests.ply_utils import TriMeshData, HalfEdgeMeshData
...
ply_dir = "./convergence_tests/data/ply_files"
vf_ply_dir = "./convergence_tests/data/vf_ply_files"
he_ply_dir = "./convergence_tests/data/he_ply_files"


class ConvTest:
    def __init__(self, mesh_path, output_dir=None, regenerate_output_dir=False):
...
# %%
# ply_path = "./convergence_tests/data/he_ply_files/sphere.ply"
vf_ply_path = "./convergence_tests/data/vf_ply_files/sphere_coarse.ply"
b = m.Brane.from_vertex_face_ply(vf_ply_path)
vf_mesh = TriMeshData.from_ply(vf_ply_path)

# %%
V, F = np.copy(vf_mesh.V), np.copy(vf_mesh.F)
Fb = b.get_faces()
Fsets = [set(face) for face in F]
Fsetsb = [set(face) for face in Fb]
count = 0
for f in range(len(F)):
    face = Fsets[f]
    for fb in range(len(F)):
        faceb = Fsetsb[fb]
        if face == faceb:
            # print(f"{(f,fb)}")
            count += 1
