...
default and with the -w option it runs the Wolff cluster algorithm. \n\
Using the options, the parameters of the simulations must be set for\n\
a new run (start=0,1). If start=2, a configuration is read from the \n\
file conf.\n";
  exit(1);
}//usage()
//=============================================
...
void simmessage(ostream& ostr){
  ...
  ostr << "\
# ########################################################################\n\
#      2d Ising Model on the square lattice                               \n\
# Run on "   <<HOST       <<" ( "<<MACH<<" ) by "<<USER<<" on "<<TIME  <<"\
# L         = "<<L        <<" (Lattice linear dimension, N=L*L)           \n\
# algorithm = "<<algorithm<<"  (0 Met/1 Wolff)                            \n\
