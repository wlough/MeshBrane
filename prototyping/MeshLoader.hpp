...
private:
  ...
  std::vector<uint3>
      F; ///< The indices of vertices in faces of the loaded mesh.
  std::vector<uint> F_hedge; ///< The half-edge index of each face.
  std::vector<uint2>
      H; ///< The indices of vertices in half-edges of the loaded mesh.
  std::vector<uint> H_face; ///< The face index of each half-edge.
  std::vector<uint>
      H_next; ///< The index of the next half-edge in each half-edge.
  std::vector<uint>
      H_twin; ///< The index of the twin half-edge in each half-edge.
  std::vector<uint> H_vertex; ///< The index of the vertex in each half-edge.
  VertexFaceList faceVertexList;
