...
load_vertex_face_list_from_ply(const std::string &filepath) {
  ...
  try {
    ...
    // }
    {
      ...
      vf_mesh.first.resize(vertices->count); // resize the vertices vector
      std::memcpy(vf_mesh.first.data(), vertices->buffer.get(),
                  numVerticesBytes);
    }
    ...
    {
      const size_t numFacesBytes = faces->buffer.size_bytes();
      vf_mesh.second.resize(faces->count);
      std::memcpy(vf_mesh.second.data(), faces->buffer.get(), numFacesBytes);
    }
  } catch (const std::exception &e) {
    std::cerr << "Caught tinyply exception: " << e.what() << std::endl;
  }
