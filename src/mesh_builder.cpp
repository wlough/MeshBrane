...
void write_simplicial_complex_data_to_ply(const SimplicialComplexData &sc_data,
  ...
  // size_t complex_dim = sc_data.complex_dimension;
  size_t complex_dim = sc_data.complex_dimension;
  // size_t num_vertices = sc_data.coords_V.rows();
  // size_t num_edges = sc_data.vertex_cycle_S1.rows();
  // size_t num_faces = sc_data.vertex_cycle_S2.rows();

  // Vertex stuff
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>
      coords_V_row_major = sc_data.coords_V;
  mesh_file.add_properties_to_element(
      "vertex", {"coords"}, tinyply::Type::FLOAT64, coords_V_row_major.rows(),
      reinterpret_cast<uint8_t *>(coords_V_row_major.data()),
      tinyply::Type::INVALID, 0);
  mesh_file.add_properties_to_element(
      "vertex", {"dart"}, tinyply::Type::INT32, sc_data.dart_V.rows(),
      reinterpret_cast<uint8_t *>(const_cast<int *>(sc_data.dart_V.data())),
      tinyply::Type::INVALID, 0);
  // Dart stuff
  mesh_file.add_properties_to_element(
      "dart", {"vertex"}, tinyply::Type::INT32, sc_data.vertex_D.rows(),
      reinterpret_cast<uint8_t *>(const_cast<int *>(sc_data.vertex_D.data())),
      tinyply::Type::INVALID, 0);

  // Boundary stuff
  mesh_file.add_properties_to_element(
      "boundary", {"dart"}, tinyply::Type::INT32, sc_data.dart_B.rows(),
      reinterpret_cast<uint8_t *>(const_cast<int *>(sc_data.dart_B.data())),
      tinyply::Type::INVALID, 0);

  if (complex_dim > 0) {
    // Edge
    Eigen::Matrix<int, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>
        vertex_cycle_S1_row_major = sc_data.vertex_cycle_S1;
    mesh_file.add_properties_to_element(
        "simplex1", {"vertex_cycle"}, tinyply::Type::INT32,
        vertex_cycle_S1_row_major.rows(),
        reinterpret_cast<uint8_t *>(vertex_cycle_S1_row_major.data()),
        tinyply::Type::UINT8, vertex_cycle_S1_row_major.cols());
    mesh_file.add_properties_to_element(
        "simplex1", {"dart"}, tinyply::Type::INT32, sc_data.dart_S1.rows(),
        reinterpret_cast<uint8_t *>(const_cast<int *>(sc_data.dart_S1.data())),
        tinyply::Type::INVALID, 0);
    // Dart stuff
    mesh_file.add_properties_to_element(
        "dart", {"dart_ve"}, tinyply::Type::INT32, sc_data.dart_ve_D.rows(),
        reinterpret_cast<uint8_t *>(
            const_cast<int *>(sc_data.dart_ve_D.data())),
        tinyply::Type::INVALID, 0);
    mesh_file.add_properties_to_element(
        "dart", {"simplex1"}, tinyply::Type::INT32, sc_data.simplex1_D.rows(),
        reinterpret_cast<uint8_t *>(
            const_cast<int *>(sc_data.simplex1_D.data())),
        tinyply::Type::INVALID, 0);
  }
  ...
  if (complex_dim > 1) {
    // Face stuff
    Eigen::Matrix<int, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>
        vertex_cycle_S2_row_major = sc_data.vertex_cycle_S2;
    mesh_file.add_properties_to_element(
        "simplex2", {"vertex_cycle"}, tinyply::Type::INT32,
        vertex_cycle_S2_row_major.rows(),
        reinterpret_cast<uint8_t *>(vertex_cycle_S2_row_major.data()),
        tinyply::Type::UINT8, vertex_cycle_S2_row_major.cols());
    mesh_file.add_properties_to_element(
        "simplex2", {"dart"}, tinyply::Type::INT32, sc_data.dart_S2.rows(),
        reinterpret_cast<uint8_t *>(const_cast<int *>(sc_data.dart_S2.data())),
        tinyply::Type::INVALID, 0);
    // Dart stuff
    mesh_file.add_properties_to_element(
        "dart", {"dart_vf"}, tinyply::Type::INT32, sc_data.dart_vf_D.rows(),
        reinterpret_cast<uint8_t *>(
            const_cast<int *>(sc_data.dart_vf_D.data())),
        tinyply::Type::INVALID, 0);
    mesh_file.add_properties_to_element(
        "dart", {"simplex2"}, tinyply::Type::INT32, sc_data.simplex2_D.rows(),
        reinterpret_cast<uint8_t *>(
            const_cast<int *>(sc_data.simplex2_D.data())),
        tinyply::Type::INVALID, 0);
  }
  ...
  if (complex_dim > 2) {
    // Cell stuff
    Eigen::Matrix<int, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>
        vertex_cycle_S3_row_major = sc_data.vertex_cycle_S3;
    mesh_file.add_properties_to_element(
        "simplex3", {"vertex_cycle"}, tinyply::Type::INT32,
        vertex_cycle_S3_row_major.rows(),
        reinterpret_cast<uint8_t *>(vertex_cycle_S3_row_major.data()),
        tinyply::Type::UINT8, vertex_cycle_S3_row_major.cols());
    mesh_file.add_properties_to_element(
        "simplex3", {"dart"}, tinyply::Type::INT32, sc_data.dart_S3.rows(),
        reinterpret_cast<uint8_t *>(const_cast<int *>(sc_data.dart_S3.data())),
        tinyply::Type::INVALID, 0);
    // Dart stuff
    mesh_file.add_properties_to_element(
        "dart", {"dart_vc"}, tinyply::Type::INT32, sc_data.dart_vc_D.rows(),
        reinterpret_cast<uint8_t *>(
            const_cast<int *>(sc_data.dart_vc_D.data())),
        tinyply::Type::INVALID, 0);
    mesh_file.add_properties_to_element(
        "dart", {"simplex3"}, tinyply::Type::INT32, sc_data.simplex3_D.rows(),
        reinterpret_cast<uint8_t *>(
            const_cast<int *>(sc_data.simplex3_D.data())),
        tinyply::Type::INVALID, 0);
  }

  mesh_file.get_comments().push_back("mesh_type SimplicialComplex");

  // Write an ply file
  mesh_file.write(outstream, use_binary);
}

EdgeFaceCellTuple cmap_to_efc_tuple(const CombinatorialMapTuple &cm_tuple) {
  // Samplesi dart_V;
  // Samplesi dart_S1;
  // Samplesi dart_S2;
  // Samplesi dart_S3;
  ...
  // Samplesi dart_vf_D;
  // Samplesi dart_vc_D;
  auto [dart_V, dart_S1, dart_S2, dart_S3, dart_B, vertex_D, simplex1_D,
        simplex2_D, simplex3_D, dart_ve_D, dart_vf_D, dart_vc_D] = cm_tuple;
  size_t Nvert = dart_V.size();
  size_t Nedge = dart_S1.size();
  size_t Nface = dart_S2.size();
  size_t Ncell = dart_S3.size();
  size_t Nbdry = dart_B.size();
  size_t Ndart = vertex_D.size();

  std::set<std::vector<int>> S3set;
  std::set<std::vector<int>> S2set;
  std::set<std::vector<int>> S1set;

  if (Ncell > 0) {
    // get ordered cell cycles and add sorted faces of cells to face set
    for (size_t i3 = 0; i3 < Ncell; ++i3) {
      std::vector<int> vertex_cycle_i3 = {
          vertex_D[dart_ve_D[dart_ve_D[dart_vf_D[dart_S3[i3]]]]],
          vertex_D[dart_S3[i3]],
          vertex_D[dart_ve_D[dart_S3[i3]]],
          vertex_D[dart_ve_D[dart_ve_D[dart_S3[i3]]]],
      };
      // std::sort(vertex_cycle_i3.begin(), vertex_cycle_i3.end());
      // vertex_cycle_S3.row(i3) = Eigen::Map<Eigen::RowVectorXi>(
      //     vertex_cycle_i3.data(), vertex_cycle_i3.size());
      S3set.insert(vertex_cycle_i3);

      // Get faces from cell and add to face set
      for (size_t j = 0; j < 4; ++j) {
        std::vector<int> vertex_cycle_i2;
        for (size_t jj = 0; jj < 4; ++jj) {
          if (jj != j) {
            vertex_cycle_i2.push_back(vertex_cycle_i3[jj]);
          }
        }
        std::sort(vertex_cycle_i2.begin(), vertex_cycle_i2.end());
        S2set.insert(vertex_cycle_i2);
      }
    }
    // add the rest of the sorted faces to face set and add sorted edges of
    // faces to edge set
    for (size_t i2 = 0; i2 < Nface; ++i2) {
      std::vector<int> vertex_cycle_i2 = {
          vertex_D[dart_S2[i2]], vertex_D[dart_ve_D[dart_S2[i2]]],
          vertex_D[dart_ve_D[dart_ve_D[dart_S2[i2]]]]};
      std::sort(vertex_cycle_i2.begin(), vertex_cycle_i2.end());
      S2set.insert(vertex_cycle_i2);
      // add sorted edges of faces to edge set
      for (size_t j = 0; j < 3; ++j) {
        std::vector<int> vertex_cycle_i1;
        for (size_t jj = 0; jj < 3; ++jj) {
          if (jj != j) {
            vertex_cycle_i1.push_back(vertex_cycle_i2[jj]);
          }
        }
        std::sort(vertex_cycle_i1.begin(), vertex_cycle_i1.end());
        S1set.insert(vertex_cycle_i1);
      }
    }
    // add the rest of the sorted edges to edge set
    for (size_t i1 = 0; i1 < Nedge; ++i1) {
      std::vector<int> vertex_cycle_i1 = {vertex_D[dart_S1[i1]],
                                          vertex_D[dart_vf_D[dart_S1[i1]]]};
      std::sort(vertex_cycle_i1.begin(), vertex_cycle_i1.end());
      S1set.insert(vertex_cycle_i1);
    }

  } else if (Nface > 0) { // end if Ncell > 0

    // get ordered face cycles and add sorted edges of faces to edge set
    for (size_t i2 = 0; i2 < Nface; ++i2) {
      std::vector<int> vertex_cycle_i2 = {
          vertex_D[dart_S3[i2]],
          vertex_D[dart_ve_D[dart_S3[i2]]],
          vertex_D[dart_ve_D[dart_ve_D[dart_S3[i2]]]],
      };
      // vertex_cycle_S2.row(i2) = Eigen::Map<Eigen::RowVectorXi>(
      //     vertex_cycle_i2.data(), vertex_cycle_i2.size());
      S2set.insert(vertex_cycle_i2);
      // Get sorted edges from face and add to edge set
      for (size_t j = 0; j < 3; ++j) {
        std::vector<int> vertex_cycle_i1;
        for (size_t jj = 0; jj < 3; ++jj) {
          if (jj != j) {
    ...
    // add the rest of the sorted edges to edge set
    for (size_t i1 = 0; i1 < Nedge; ++i1) {
      std::vector<int> vertex_cycle_i1 = {vertex_D[dart_S2[i1]],
                                          vertex_D[dart_vf_D[dart_S2[i1]]]};
      std::sort(vertex_cycle_i1.begin(), vertex_cycle_i1.end());
      S1set.insert(vertex_cycle_i1);
    } // end for i1
  } else if (Nedge > 0) { // end if Nface > 0
    // get ordered edge cycles
    for (size_t i1 = 0; i1 < Nedge; ++i1) {
      std::vector<int> vertex_cycle_i1 = {vertex_D[dart_S1[i1]],
                                          vertex_D[dart_ve_D[dart_S1[i1]]]};
      // vertex_cycle_S1.row(i1) = Eigen::Map<Eigen::RowVectorXi>(
      //     vertex_cycle_i1.data(), vertex_cycle_i1.size());
      S1set.insert(vertex_cycle_i1);
    } // end for i1
  } // end if Nedge > 0
  ...
  Nface = S2set.size();
  Nedge = S1set.size();
  Samples2i vertex_cycle_S1(Nedge, 2);
  Samples3i vertex_cycle_S2(Nface, 3);
  Samples4i vertex_cycle_S3(Ncell, 4);
  // Add cells in cell set to cell matrix
  while (!S3set.empty()) {
    std::vector<int> vertex_cycle_i3 = *S3set.begin();
    S3set.erase(S3set.begin());
    vertex_cycle_S3.row(Ncell - S3set.size() - 1) =
        Eigen::Map<Eigen::RowVectorXi>(vertex_cycle_i3.data(),
                                       vertex_cycle_i3.size());
  } // end while S3set not empty
  // Add faces in face set to face matrix
  while (!S2set.empty()) {
    std::vector<int> vertex_cycle_i2 = *S2set.begin();
    S2set.erase(S2set.begin());
    vertex_cycle_S2.row(Nface - S2set.size() - 1) =
        Eigen::Map<Eigen::RowVectorXi>(vertex_cycle_i2.data(),
                                       vertex_cycle_i2.size());
  } // end while S2set not empty
  // Add edges in edge set to edge matrix
  while (!S1set.empty()) {
    std::vector<int> vertex_cycle_i1 = *S1set.begin();
    S1set.erase(S1set.begin());
    vertex_cycle_S1.row(Nedge - S1set.size() - 1) =
        Eigen::Map<Eigen::RowVectorXi>(vertex_cycle_i1.data(),
                                       vertex_cycle_i1.size());
  } // end while S1set not empty
  return std::make_tuple(vertex_cycle_S1, vertex_cycle_S2, vertex_cycle_S3);
}

SimplicialComplexData
load_implicial_complex_data_from_ply(const std::string &filepath,
                                     const bool preload_into_memory,
                                     const bool verbose) {
  std::streambuf *oldCoutStreamBuf = nullptr;
  std::ofstream nullStream;
  ...
  // it into a stream is a net win for parsing speed, about 40% faster.
  if (preload_into_memory) {
    byte_buffer = read_file_binary(filepath);
    file_stream.reset(
        new memory_stream((char *)byte_buffer.data(), byte_buffer.size()));
  ...
  // data as structured/typed byte buffers.
  std::shared_ptr<tinyply::PlyData> coords_Vplydata, dart_Vplydata,
      vertex_cycle_S1plydata, dart_S1plydata, vertex_cycle_S2plydata,
      dart_S2plydata, vertex_cycle_S3plydata, dart_S3plydata, dart_Bplydata,
      simplices_Dplydata, dart_ve_Dplydata, dart_vf_Dplydata, dart_vc_Dplydata;
  ...
  // the data. Providing a list size hint (the last argument) is a 2x
  // performance improvement. If you have arbitrary ply files, it is best
  // to
  // leave this 0.
  try {
    coords_Vplydata =
        file.request_properties_from_element("vertex", {"coords"}, 3);
  } catch (const std::exception &e) {
    std::cerr << "tinyply exception: " << e.what() << std::endl;
  }

  try {
    dart_Vplydata = file.request_properties_from_element("vertex", {"dart"});
  } catch (const std::exception &e) {
    std::cerr << "tinyply exception: " << e.what() << std::endl;
  }

  try {
    vertex_cycle_S1plydata =
        file.request_properties_from_element("simplex1", {"vertex_cycle"}, 2);
  } catch (const std::exception &e) {
    std::cerr << "tinyply exception: " << e.what() << std::endl;
  }
  try {
    dart_S1plydata = file.request_properties_from_element("simplex1", {"dart"});
  } catch (const std::exception &e) {
    std::cerr << "tinyply exception: " << e.what() << std::endl;
  }

  try {
    vertex_cycle_S2plydata =
        file.request_properties_from_element("simplex2", {"vertex_cycle"}, 3);
  } catch (const std::exception &e) {
    std::cerr << "tinyply exception: " << e.what() << std::endl;
  }
  try {
    dart_S2plydata = file.request_properties_from_element("simplex2", {"dart"});
  } catch (const std::exception &e) {
    std::cerr << "tinyply exception: " << e.what() << std::endl;
  }

  try {
    vertex_cycle_S3plydata =
        file.request_properties_from_element("simplex3", {"vertex_cycle"}, 4);
  } catch (const std::exception &e) {
    std::cerr << "tinyply exception: " << e.what() << std::endl;
  }
  try {
    dart_S3plydata = file.request_properties_from_element("simplex3", {"dart"});
  } catch (const std::exception &e) {
    std::cerr << "tinyply exception: " << e.what() << std::endl;
  }

  try {
    dart_Bplydata = file.request_properties_from_element("boundary", {"dart"});
  } catch (const std::exception &e) {
    std::cerr << "tinyply exception: " << e.what() << std::endl;
  }
  ...
  try {
    simplices_Dplydata =
        file.request_properties_from_element("dart", {"simplices"}, 4);
  } catch (const std::exception &e) {
    std::cerr << "tinyply exception: " << e.what() << std::endl;
  }
  ...
  // Convert to positions to Samples3d
  const size_t numVertices = coords_Vplydata->count;
  sc_data.coords_V.resize(numVertices, 3);
  const double *coords_Vbuffer =
      reinterpret_cast<const double *>(coords_Vplydata->buffer.get());
  for (size_t i = 0; i < numVertices; ++i) {
    sc_data.coords_V(i, 0) = coords_Vbuffer[3 * i + 0]; // x
    sc_data.coords_V(i, 1) = coords_Vbuffer[3 * i + 1]; // y
    sc_data.coords_V(i, 2) = coords_Vbuffer[3 * i + 2]; // z
  }

  // Convert faces to Samples3i
  // const size_t numFaces = faces->count;
  // V_cycle_F.resize(numFaces, 3);
  // const int *faceBuffer = reinterpret_cast<const int
  // *>(faces->buffer.get()); for (size_t i = 0; i < numFaces; ++i) {
  //   V_cycle_F(i, 0) = faceBuffer[3 * i + 0]; // vertex index 1
  //   V_cycle_F(i, 1) = faceBuffer[3 * i + 1]; // vertex index 2
  //   V_cycle_F(i, 2) = faceBuffer[3 * i + 2]; // vertex index 3
  // }
