...
class Brane:
    def __init__(self, vertices, faces):
        ...
        self.V_scalar = self.get_angle_defects()
    ...
    def mean_pose(self, vertex_list):
        ...
        associated with vertices in vertex_list"""
        iters = 4
        G = np.array([self.pq[i] for i in vertex_list])
        Nsamps = len(vertex_list)
        mu_g = np.array(G[0])
    ...
    def mean_quaternion(self, vertex_list):
        """computes the SE3-valued mean of the euclidean transformations
        associated with vertices in vertex_list"""
        iters = 4
        G = np.array([self.pq[i, 3:] for i in vertex_list])
        Nsamps = len(vertex_list)
        mu_g = np.array(G[0])
        ...
        for iter in range(iters):
            ...
            for g in G:
                ...
                Psi += log_quaternion(mu_g_inv_g) / Nsamps
                # Psi += log_se3_quaternion(mul_se3_quaternion(g, mu_g_inv)) / Nsamps
            mu_g = mul_quaternion(mu_g, exp_quaternion(Psi))
            # mu_g = mul_se3_quaternion(exp_se3_quaternion(Psi), mu_g)
        return mu_g
