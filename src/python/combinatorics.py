...
class CombinatorialSimplex:
    ...
    def __init__(self, points_list):
        self.parity = parity_of_argsort(points_list)
        self.points = frozenset(points_list)
        self.str = str(self.list)
    ...
    def __repr__(self):
        return f"CombinatorialSimplex({self.list})"
    ...
    @classmethod
    def from_set_and_parity(cls, points, parity):
        if parity:
            return cls(sorted(points))
        else:
            P = sorted(points)
            P[0], P[1] = P[1], P[0]
            return cls(P)
