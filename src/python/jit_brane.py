...
fac_plye_numba_type = Array(
    from_dtype(np.dtype([("vertex_indices", "int32", (3,))])), 1, "C"
)
hedg_plye_numba_type = Array(
    from_dtype(np.dtype([("vertex_indices", "int32", (2,))])), 1, "C"
)

# xyz_numpy_dtype = np.dtype([("x", "float64"), ("y", "float64"), ("z", "float64")])
# xyz_numpy_dtype = np.float64
# xyz_numpy_dtype = np.dtype([("x", np.float64), ("y", np.float64), ("z", np.float64)])
# xyz_numba_type = Array(from_dtype(xyz_numpy_dtype), 1, "C")

vertex_index_numba_type = int32
halfedge_index_numba_type = int32
face_index_numba_type = int32
boundary_index_numba_type = int32
xyz_numba_type = Array(float64, 1, "C")
...
class HalfEdgeMeshBase:
    ...
    def xyz_coord_V(self, value):
        tof = typeof(value)
        if tof == xyz_coord_V_numba_type:
            self._xyz_coord_V = value
        elif hasattr(value, "__iter__"):
            numba_dict = Dict.empty(
                key_type=vertex_index_numba_type, value_type=xyz_numba_type
            )
            for _k, v in enumerate(value):
                k = vertex_index_numba_type(_k)
                numba_dict[k] = np.array(v, dtype=np.float64)
            self._xyz_coord_V = numba_dict
        else:
            raise TypeError("xyz_coord_V must be a dictionary or an iterable.")
