...
@njit
def _jit_vf_samples_to_he_samples(V, F):
    # (V, F) = source_samples
    Nfaces = len(F)
    Nvertices = len(V)
...
@njit
def _jit_refine_icososphere(selfV, selfF, selfr):
    F = []
    v_midpt_vv = dict()
    for tri in selfF[-1]:
        v0, v1, v2 = tri
        ...
        if v01 is None:
            v01 = len(selfV)
            xyz01 = (selfV[v0] + selfV[v1]) / 2
            xyz01 *= selfr / np.linalg.norm(xyz01)
            selfV.append(xyz01)
            v_midpt_vv[(v0, v1)] = v01
            v_midpt_vv[(v1, v0)] = v01
        if v12 is None:
            v12 = len(selfV)
            xyz12 = (selfV[v1] + selfV[v2]) / 2
            xyz12 *= selfr / np.linalg.norm(xyz12)
            selfV.append(xyz12)
            v_midpt_vv[(v1, v2)] = v12
            v_midpt_vv[(v2, v1)] = v12
        if v20 is None:
            v20 = len(selfV)
            xyz20 = (selfV[v2] + selfV[v0]) / 2
            xyz20 *= selfr / np.linalg.norm(xyz20)
            selfV.append(xyz20)
    ...
    selfF.append(F)
    # self_num_vertices.append(len(selfV))
    # if convert_to_half_edge:
    #     print("Converting to half-edge mesh...")
    #     selfv2h.append(VertTri2HalfEdgeConverter.from_source_samples(*selfVF()))
    # return selfV, F
...
#     return int32(min(v0, v1) * 1000000 + max(v0, v1))
...
#     v_midpt_vv = Dict.empty(key_type=int32, value_type=int32)
#     for tri in Fm1:
#         v0, v1, v2 = tri
