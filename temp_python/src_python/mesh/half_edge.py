import numpy as np
from temp_python.lib.cply_tools import MeshConverter
...
class HalfEdgeMesh:
    ...
    - From an array of vertex positions and an array of face vertices:
        HalfEdgeMesh.from_vf_samples(xyz_coord_V, V_of_F)
    - From a ply file (binary/ascii) containing vertex/face data:
        HalfEdgeMesh.from_vf_ply(ply_path)
        * See MeshConverter for more details about ply format.
    ...
    def init_empty(cls):
        return cls(
            np.zeros((0, 3), dtype=FLOAT_TYPE),
            np.zeros(0, dtype=INT_TYPE),
            np.zeros(0, dtype=INT_TYPE),
            np.zeros(0, dtype=INT_TYPE),
            np.zeros(0, dtype=INT_TYPE),
            np.zeros(0, dtype=INT_TYPE),
    ...
    @classmethod
    def from_vf_samples(cls, xyz_coord_V, V_of_F, *args, **kwargs):
        """
        ...
        xyz_coord_V : list of numpy.array
            xyz_coord_V[i] = xyz coordinates of vertex i
        V_of_F : list of lists of integers
            V_of_F[j] = [v0, v1, v2] = vertices in face j.
        ...
        """
        return cls(
            *MeshConverter.from_vf_samples(xyz_coord_V, V_of_F).he_samples,
            *args,
            **kwargs,
        )
    ...
    def from_vf_ply(cls, ply_path, *args, **kwargs):
        ...
        """
        return cls(*MeshConverter.from_vf_ply(ply_path).he_samples, *args, **kwargs)
    ...
    def from_he_ply(cls, ply_path, *args, **kwargs):
        ...
        """
        return cls(*MeshConverter.from_he_ply(ply_path).he_samples, *args, **kwargs)
    ...
    def load(cls, **kwargs):
        ...
        elif "xyz_coord_V" in kwargs and "V_of_F" in kwargs:
            return cls.from_vf_samples(
                kwargs["xyz_coord_V"], kwargs["V_of_F"], **kwargs
            )
    ...
    def V_of_H(self):
        ...
        # )
        return self.v_origin_h([range(self.num_half_edges), self.h_twin_H]).T

    @property
    def V_of_E(self):
        V_of_H = self.v_origin_h([range(self.num_half_edges), self.h_twin_H]).T
        return V_of_H[V_of_H[:, 0] < V_of_H[:, 1]]
    ...
    # To be deprecated ###################################
    #######################################################
    def _generate_V_nearest_v_clockwise(self, v, h_start=None):
...
class HalfEdgeCurve:
    ...
    def h_next_h(self, h):
        """
        Get index of the next half-edge after h in the curve. Returns -1 if h is the final half-edge in the curve.
        Args:
            h (int): half-edge index
